    ...
    return new Promise(function (resolve, reject) {
        ...
        fs.createReadStream(peopleFilename)
            ...
            .on("end", async () => {
                ...
                const columns = [
                    "id", "titre", "titreEN", "etablissement", "defense", "start", "authorFirstName", "authorLastName", "authorIdHal", "authorHAL", "authorTeam",
                    "authorWebpage", "discipline", "status", "school", "keywords", "keywordsEN", "rameau", "partners", "halId", "halUri", "halFile",
                    "jury1FirstName", "jury1LastName", "jury1role",
                    "jury2FirstName", "jury2LastName", "jury2role",
                    "jury3FirstName", "jury3LastName", "jury3role",
                    "jury4FirstName", "jury4LastName", "jury4role",
                    "jury5FirstName", "jury5LastName", "jury5role",
                    "jury6FirstName", "jury6LastName", "jury6role",
                    "jury7FirstName", "jury7LastName", "jury7role",
                    "jury8FirstName", "jury8LastName", "jury8role",
                    "jury9FirstName", "jury9LastName", "jury9role",
                    "jury10FirstName", "jury10LastName", "jury10role",
                ];
                const stringifier = stringify({ header: true, columns: columns });
                ...
                await get(`https://theses.fr/api/v1/theses/recherche/?${params}`).then(res => {
                    ...
                    const promises = res.data.theses.map(async (thesis) => {
                        ...
                        async function getAuthorHalInfo() {
                            ...
                            await get(halApiRequest).then(hal => {
...
function getDatasets(inputDatasetFilename, datasetFilename) {
    ...
    fs.createReadStream(inputDatasetFilename)
        ...
        .on("end", async () => {
            ...
            const columns = ["name", "short_name", "doi", "url", "date", "project", "theme", "teams", "image_url", "downloads"];
            const stringifier = stringify({ header: true, columns: columns });
                ...
                if (url.includes("zenodo")) {
                    ...
                    if (url.includes("dataverse") || url.includes("entrepot.recherche.data.gouv.fr")) {
                        ...
                        await get(url).then((response) => {
                            ...
                            if (response.status == 200) {
                                ...
                                const downloads = $('.metrics-count-block')
                                    .map((_, block) => {
                                        const $block = $(block);
                                        return $block.text().substring(0, $block.text().indexOf(" Downloads")).replaceAll(",", "")
                                    })
                                    .toArray()[0];
                        ...
                        if (url.includes("figshare")) {
                            ...
                            var downloadValue = 0;
                            await get(`https://stats.figshare.com/total/article/${dataId}`).then(res => {
                            ...
                            if (url.includes("mendeley")) {
                                ...
                                const dataId = url.substring(url.lastIndexOf("/") + 1)
                                var downloadValue = 0;
                                await get(`https://api.plu.mx/widget/elsevier/artifact?type=mendeley_data_id&id=${dataId}&hidePrint=true&site=plum&href=https://plu.mx/plum/a?mendeley_data_id=${dataId}`).then(res => {
