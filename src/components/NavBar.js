...
import { Nav, NavBarContainer, NavBarLogo, MobileIcon, NavBarMenu, NavItem, NavLink, NavMenuButton, NavMenu, NavSubMenu, NavMenuItem } from './styles/NavBar.styled'
...
export const NavBar = ({ title, menus, lastigLogo, useLocale, team }) => {
        ...
        if (menu.subMenu && menu.subMenu.length > 0) {
            ...
            if (topMenu) {
                ...
                return (
                    <NavMenu key={`${team}-${menu.link}-${menu.name}`} team={team} align="center" onClick={(e) => {
                        ...
                        menuButton={<NavMenuButton onClick={(e) => {
                            e.stopPropagation();
                            e.keepOpen = true;
                        }}>{trans(menu.name)}</NavMenuButton>}>{subMenuItems}
                    </NavMenu>
                )
            }
            ...
            return (
                <NavSubMenu key={`${team}-${menu.link}-${menu.name}`} align="center" team={team} onClick={(e) => {
                    ...
                    e.keepOpen = true;
                }} label={trans(menu.name)}>{subMenuItems}
                </NavSubMenu>
            )
        }
    ...
    const navMenuItems = menus.map((menu) => {
        ...
        return (<NavItem key={menu.name}><NavLink to={menu.link} >{trans(menu.name)}</NavLink></NavItem>)
    })
    return (
        <>
