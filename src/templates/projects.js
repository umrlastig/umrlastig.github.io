...
export default function ProjectsPage({ data, pageContext }) {
    ...
    const colorDomain = ["H2020","ERA4CS","Horizon Europe","ENIAC","ANR","TOSCA","AID","iSite","PEPR","FUI",'AAP Excellences',"TIGA","PEPS","ACTE","GEOVIS","MEIG","STRUDEL"]
    const projects = data.allProjectsCsv.nodes.map((node) => {
    ...
    projects.sort((a, b) => a.theme.localeCompare(b.theme) || a.start_date-b.start_date || a.end_date-b.end_date || a.name.localeCompare(b.name))
    const sorted_projects = projects.map((p,index) => {return {index: index, ...p}})
    sorted_projects.forEach((p)=>console.log(p.index,p.theme,p.start_date,p.end_date,p.name))
    const teams = sorted_projects.flatMap((p)=>p.teams.map((t,index)=>({end_date:p.end_date,team:t,index:p.index,dx:-(index+1)*10})))
    ...
    const sorted_themes = themes.map((t)=>{
        ...
        return {theme:t, start_date:first_date, end_date: last_date, index_1: first_index, index_2: last_index, index_middle: Math.round((first_index+last_index)/2)}
    })
    sorted_themes.forEach((t)=>console.log(t.theme,t.start_date,t.end_date,t.index_1,t.index_2,t.index_middle))
    return (
        <Layout pageTitle={`${isLastigPage ? 'LASTIG' : team} Projects`}>
            <h1>{trans(`${isLastigPage ? 'LASTIG' : team} Projects`)}</h1>
                ...
                <ProjectsPlot
                    options={{
                        ...
                        marks: [
                            ...
                            Plot.text(sorted_projects, {x: "middle_date", y: "index", text: (d) => d.name, lineAnchor: "middle",fontWeight:'bold'}),
                            dxValues.map((value)=>Plot.dot(teams.filter(({dx})=>dx===value), {x: (t)=>t.end_date, y: "index", dx: value, dy:-10, fill:"team"})),
                            Plot.text(sorted_themes, {x: "start_date", y: "index_middle", text: (d) => d.theme, lineAnchor: "middle",rotate:-90,dx:10, fontWeight:'bold'}),
...
export const Head = ({ pageContext }) => <Seo title={`${pageContext.team.length > 1 ? 'LASTIG' : pageContext.team} Projects`} />
