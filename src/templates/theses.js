...
export default function thesesPage({ data, pageContext }) {
    ...
    const ongoingNodes = data.allThesesCsv.nodes.filter((node) => node.status === "ongoing");
    const defendedNodes = data.allThesesCsv.nodes.filter((node) => node.status === "defended");
        ...
        <Layout pageTitle={`${isLastigPage ? 'LASTIG' : team} Theses`}>
                ...
                <tr key={index+"="+node.id}>
                    ...
                    <td>
                        <span key={`${node.id}-author`}>
                            {node.authorWebpage ? <a href={node.authorWebpage} key={`${node.id}-hal`}>{`${node.authorFirstName} ${node.authorLastName}`}</a> : `${node.authorFirstName} ${node.authorLastName}`}
                        </span>
                        </td>
                    <td>
                        <a href={`https://theses.fr/${node.id}`} key={`${node.id}-title`}>{node.titre}</a>
                    </td>
                </tr>
            )})}
            <tr key="defended">
                <td><h2>Defended theses</h2></td>
            </tr>
            {defendedNodes.map((node, index) => {
                ...
                <tr key={index+"="+node.id}>
                    ...
                    <td key={index+"-file"}><ImageLink>{node.halFile && <a href={node.halFile} aria-label='Main document in HAL'><FaFilePdf /></a>}</ImageLink></td>
                </tr>
            )})}
            </tbody>
            </table>
...
export const Head = ({pageContext}) => <Seo title={`${pageContext.team.length > 1 ? 'LASTIG' : pageContext.team} Theses`} />
